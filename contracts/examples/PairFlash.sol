...
import '@ethaf/ethaf-core/contracts/interfaces/callback/IEthAfFlashCallback.sol';
import '@ethaf/ethaf-core/contracts/libraries/LowGasSafeMath.sol';
...
/// @notice An example contract using the ETH AF flash function
contract PairFlash is IEthAfFlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `ethafFlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        IEthAfPool pool = IEthAfPool(PoolAddress.computeAddress(factory, poolKey));
